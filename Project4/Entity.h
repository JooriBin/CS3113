...
enum AIType { GUARD, WALKER, JUMPER };
...
private:
    ...
    //set getters
    bool m_collided_enemy_top = false; //top of enemy
    bool m_collided_enemy_bottom = false; //bottom of enemy
    bool m_collided_enemy_right = false;  //right of enemy
    bool m_collided_enemy_left = false;  //left of enemy
...
public:
    ...
    void const check_collision_enemy(Entity* other);
    ...
    void move_left() {
        m_movement.x = -1.0f; face_left();   m_scale.x = -1.0f;
    }
    void move_right() { m_movement.x = 1.0f;  face_right(); m_scale.x = 1.0f;  }
    ...
    void const set_init_pos(float pos) { m_walker_init_pos_x = pos; }
