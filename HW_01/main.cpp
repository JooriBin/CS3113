...
constexpr glm::vec3 INIT_SCALE = glm::vec3(2.0f, 2.98f, 0.0f),
                    ...
                    INIT_POS_CRASH_TAWNA = glm::vec3(-2.0f, 0.0f, 0.0f);
constexpr float ROT_INCREMENT = 1.0f;
constexpr float speed = 0.0001f;
glm::vec3 accumulated_position_crash(INIT_POS_CRASH.x, INIT_POS_CRASH.y, 0.0f);
glm::vec3 accumulated_position_tawna(INIT_POS_CRASH_TAWNA.x, INIT_POS_CRASH_TAWNA.y, 0.0f);
...
void update()
{
    ...
    g_rotation_tawna.y += -1 * ROT_INCREMENT * delta_time;
    accumulated_position_crash.x += speed;
    accumulated_position_crash.y = 1;
    ...
    /* Transformations */
    g_crash_matrix = glm::translate(g_crash_matrix, accumulated_position_crash);
    g_crash_matrix = glm::rotate(g_crash_matrix,
                                 g_rotation_crash.y,
                                 glm::vec3(0.0f, 1.0f, 0.0f));
    g_crash_matrix = glm::scale(g_crash_matrix, INIT_SCALE * 1.2f);
    
    g_tawna_matrix = glm::translate(g_tawna_matrix, accumulated_position_tawna);
    g_tawna_matrix = glm::rotate(g_tawna_matrix,
                                  g_rotation_tawna.y,
                                  glm::vec3(0.0f, 1.0f, 0.0f));
    g_tawna_matrix = glm::scale(g_tawna_matrix, INIT_SCALE);
}
